(() => {
  ...
  if (window[INSTALL_KEY]) {
    ...
    // 版本不同，或不健康（timer/observer/root 丢失）时，销毁旧实例并重新初始化。
    // 版本相同且健康时，只 refresh 后返回，避免重复初始化。
    const needsRebuild =
      !api ||
      api.version !== SCRIPT_VERSION ||
      (typeof api.isHealthy === "function" && !api.isHealthy());
    if (needsRebuild) {
  ...
  function findInlineMount() {
    ...
    const findComposerFooterMount = () => {
      const footerSelector = ".composer-footer, [class*='_footer_'], .composer-surface-chrome [class*='footer']";
    ...
    const findStructuralMountForControl = (control) => {
      const footer = control.closest(".composer-footer, [class*='_footer_'], [class*='footer']");
      if (footer) {
        const footerRect = footer.getBoundingClientRect();
      ...
      while (current && current !== document.body) {
        ...
        if (rect.height === 0 || rect.width === 0) { current = current.parentElement; continue; }
  ...
  function mountRoot(root) {
    ...
    if (!mount || !mount.parent || root.contains(mount.parent)) {
      ...
      if (root.parentNode !== document.body) document.body.appendChild(root);
      // inline 挂载点未就绪时回退到 floating 显示，避免 meter 永久隐藏。
      root.dataset.placement = "floating";
      applyFloatingUiState(root);
      closeSpendHistory();
  ...
  function scanCapturedContextUsage(activeConversationId) {
    if (!state.capturedUsage) return null;
